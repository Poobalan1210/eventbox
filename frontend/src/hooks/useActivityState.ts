...
export interface UseActivityStateOptions {
  eventId?: string;
  onActivityActivated?: (activity: Activity) => void;
  onActivityDeactivated?: (activityId: string) => void;
  onActivityUpdated?: (activity: Activity) => void;
  onWaitingStateChanged?: (isWaiting: boolean, message: string) => void;
}
 ...
 */
export const useActivityState = (options: UseActivityStateOptions = {}) => {
  ...
  useEffect(() => {
    const cleanup = on('activity-updated', (payload: ActivityUpdatedPayload) => {
      // Only update if this is for our event (or no event filter specified)
      if (!eventId || payload.eventId === eventId) {
        setState((prev) => {
  ...
  const getActiveActivityType = useCallback((): Activity['type'] | null => {
