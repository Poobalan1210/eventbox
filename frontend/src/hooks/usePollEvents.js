 ...
 */
export const usePollEvents = (options = {}) => {
    ...
    useEffect(() => {
        const cleanup = on('poll-vote-submitted', (payload) => {
            // Only update if this is for our activity (or no activity filter specified)
            if (!activityId || payload.activityId === activityId) {
    ...
    const submitVote = useCallback((selectedOptionIds) => {
        if (!state.activityId) {
    ...
    const getOptionPercentage = useCallback((optionId) => {
    ...
    const getLeadingOption = useCallback(() => {
        if (state.options.length === 0)
            return null;
        return state.options.reduce((prev, current) => current.voteCount > prev.voteCount ? current : prev);
    }, [state.options]);
    return {
        ...state,
        submitVote,
