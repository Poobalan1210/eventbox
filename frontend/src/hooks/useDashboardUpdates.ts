...
import { QuizCardData } from '../components/QuizCard';

interface DashboardUpdateHandlers {
  onQuizStatusChanged?: (eventId: string, status: string, timestamp: number) => void;
  onQuizMetadataUpdated?: (eventId: string, updates: Partial<QuizCardData>) => void;
  onParticipantCountUpdated?: (eventId: string, count: number) => void;
}
...
export function useDashboardUpdates(
  organizerId: string,
  handlers: DashboardUpdateHandlers
) {
  ...
  const handleStatusChange = useCallback(
    (payload: QuizStatusChangedPayload) => {
  ...
  const handleMetadataUpdate = useCallback(
    (payload: QuizMetadataUpdatedPayload) => {
      console.log('Quiz metadata updated:', payload);
      if (handlers.onQuizMetadataUpdated) {
        const updates: Partial<QuizCardData> = {
          lastModified: payload.lastModified,
        };
        if (payload.name) updates.name = payload.name;
        if (payload.topic) updates.topic = payload.topic;
