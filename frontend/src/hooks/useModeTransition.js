...
export function useModeTransition({ eventId, currentStatus, onStatusChange, }) {
    ...
    const validateForLive = async () => {
        ...
        try {
            ...
            for (let i = 0; i < questions.length; i++) {
                ...
                const hasCorrectAnswer = question.options.some((opt) => opt.id === question.correctOptionId);
                if (!hasCorrectAnswer) {
    ...
    const transitionTo = useCallback(async (targetStatus) => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to transition mode';
